...
- **2페이지 UI/UX 패리티**: `M2hPage`와 `H2mPage`는 사용자에게 같은 앱처럼 보여야 하므로, 공유되는 상호작용 패턴(파일 드래그 오버 하이라이트, 드롭 수용/거부 피드백, 복사·다운로드 버튼 상태 등)은 한쪽만 고치지 않는다. 한쪽 페이지의 상호작용·시각 반응을 바꿀 때는 다른 쪽도 동일한 클래스·타이밍·동작으로 맞추고, 브라우저에서 양쪽의 실제 렌더링이 일치하는지 확인한다.
