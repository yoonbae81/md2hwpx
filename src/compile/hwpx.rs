...
pub fn rewrite_hwpx_bytes(
    ...
    // 방어 깊이: 호출 순서(compile_hwpx_labeled이 read_template_parts_bytes에서
    // 이미 검증)와 무관하게 재작성 직전에 패키지 상한·경로 탈출·암호화 검사를
    // 스스로 다시 수행한다.
    check_package_size(&mut zin, "template")?;
...
mod tests {
    use super::*;
    use crate::shared::zip_read::test_fixtures::fixture_hwpx;
    use std::io::{Cursor, Read};

    #[test]
    fn rewrite_hwpx_bytes_validates_package_independently() {
        // 호출 순서에 기대지 않고 zip-slip/상한/암호화 검사를 스스로 수행한다.
        let mut buf = Vec::new();
        {
            use zip::write::SimpleFileOptions;
            use zip::ZipWriter;
        ...
        let err = rewrite_hwpx_bytes(&buf, b"<r/>", None).unwrap_err();
        assert!(err.0.contains("path traversal"), "{}", err.0);
    }

    #[test]
    fn rewrite_hwpx_bytes_replaces_section0_and_keeps_other_entries() {
        let template = fixture_hwpx(&[("Contents/section0.xml", "<hs/>")]);
        let out = rewrite_hwpx_bytes(&template, b"<replaced/>", Some(b"<header/>")).unwrap();
        let mut zin = ZipArchive::new(Cursor::new(&out)).unwrap();
        let mut read = |name: &str| {
            let mut buf = Vec::new();
            zin.by_name(name).unwrap().read_to_end(&mut buf).unwrap();
            buf
        };
        assert_eq!(read("Contents/section0.xml"), b"<replaced/>");
        assert_eq!(read("Contents/header.xml"), b"<header/>");
        assert_eq!(read("mimetype"), HWPX_MIMETYPE.as_bytes());
    }
}
