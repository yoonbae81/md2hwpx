...
//!   9. 빈 줄 컴팩션 (spacing.rs) — 런 사이 한 칸, 런 내부/문서 가장자리는
//!      붙임(종료 개행은 유지). 마지막 단계여야 한다: 앞 규칙들의 판정
//!      (불릿 맥락의 빈 줄 유지, 표 행-구분행 인접)은 컴팩션 전 원문으로
//!      이루어지고, 컴팩션은 줄 모양만 바꾸지 않는 빈 줄 배치 조정이다.
//!
...
pub fn postprocess_markdown(markdown: &str) -> String {
    ...
    // 빈 줄 컴팩션은 마지막: ``` 펜스 내부를 뺀 나머지를 정규 형태
    // (런 사이 한 칸, 런 내부 붙임)로 끌어간다.
    let lines = apply_outside_code(lines, spacing::compact_blank_lines);
    lines.join("\n")
}
