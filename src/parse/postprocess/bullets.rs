...
fn parse_bullet_line(line: &str) -> Option<BulletLine> {
    ...
    // 하이라이트 경계/가로선 `---`는 불릿이 아니라 방언 구분 줄이다. 불릿으로
    // 보면 복원된 경계가 `  - --`로 재작성되어 왕복이 깨진다.
    if line.trim() == dialect::HRULE {
        return None;
    }
...
mod tests {
    ...
    #[test]
    fn highlight_boundary_dashes_pass_through_untouched() {
        // 박스 복원 패스가 내놓은 하이라이트 경계 `---`는 불릿 후보가
        // 아니므로 그대로 통과한다(불릿으로 보면 `  - --`로 재작성되어
        // 왕복이 깨진다). 사이 항목은 첫 문맥 불릿 규약대로 2단계가 된다.
        let src = lines(&["---", "- 핵심 요약", "---"]);
        assert_eq!(
            restore_bullet_levels(&src.join("\n")),
            lines(&["---", "  - 핵심 요약", "---"]).join("\n")
