...
/// 구조 폭탄 방어: 파싱 중 누적하는 요소 수와 텍스트 바이트 상한.
/// zip_read.rs의 decompressed 크기 상한(엔트리 64MiB)과 짝을 이루며,
/// 정상 문서보다 훨씬 큰 값이다.
const MAX_TOTAL_ELEMENTS: usize = 2_000_000;
const MAX_TOTAL_TEXT_BYTES: usize = 64_000_000;
        ...
        match event {
            Event::Start(e) => {
                ...
                if total_elements > MAX_TOTAL_ELEMENTS {
                    return err(format!(
                        "XML document has too many elements (limit {MAX_TOTAL_ELEMENTS})."
                    ));
                }
            ...
            Event::Text(t) => {
                let s = t
                    ...
                    .into_owned();
                total_text_bytes += s.len();
                if total_text_bytes > MAX_TOTAL_TEXT_BYTES {
                    return err(format!(
                        "XML document has too much text (limit {MAX_TOTAL_TEXT_BYTES} bytes)."
                    ));
                }
...
<hp:p paraPrIDRef=\"1\"><hp:run charPrIDRef=\"2\"><hp:t>가&amp;b&lt;&gt;</hp:t></hp:run></hp:p></hp:hs>"
    ...
    #[test]
    fn rejects_document_exceeding_element_cap() {
        // 깊이 상한(512)에는 못 미치지만 요소 수 상한을 넘는 넓은 문서.
        let mut data = Vec::with_capacity(MAX_TOTAL_ELEMENTS * 4 + 8);
        data.extend_from_slice(b"<r>");
        for _ in 0..MAX_TOTAL_ELEMENTS + 1 {
            data.extend_from_slice(b"<a/>");
        }
        data.extend_from_slice(b"</r>");
        let err = parse(&data).unwrap_err();
        assert!(err.0.contains("too many elements"), "{}", err.0);
    }

    #[test]
    fn rejects_document_exceeding_text_cap() {
        // 텍스트 누적 바이트가 상한을 넘으면 깨끗한 오류로 거부한다.
        let mut data = Vec::with_capacity(MAX_TOTAL_TEXT_BYTES + 8);
        data.extend_from_slice(b"<r>");
        data.extend_from_slice(&vec![b'a'; MAX_TOTAL_TEXT_BYTES + 1]);
        data.extend_from_slice(b"</r>");
        let err = parse(&data).unwrap_err();
        assert!(err.0.contains("too much text"), "{}", err.0);
    }

    #[test]
    fn parses_wide_document_just_under_element_cap() {
        // 상한 바로 아래는 정상 문서로 취급해 통과시킨다.
        let mut data = Vec::with_capacity(MAX_TOTAL_ELEMENTS * 4);
        data.extend_from_slice(b"<r>");
        for _ in 0..MAX_TOTAL_ELEMENTS - 1 {
            data.extend_from_slice(b"<a/>");
        }
        data.extend_from_slice(b"</r>");
        let root = parse(&data).unwrap();
        assert_eq!(root.borrow().children.len(), MAX_TOTAL_ELEMENTS - 1);
    }
}
