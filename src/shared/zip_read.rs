...
pub const HWPX_MAX_ENTRY_BYTES: u64 = 64 * 1024 * 1024;

pub fn check_package_size<R: Read + std::io::Seek>(
    package: &mut ZipArchive<R>,
    label: &str,
) -> Result<()> {
    // 선언된 압축 해제 크기의 합 및 단일 엔트리 상한/보안 검사
    let mut total: u64 = 0;
    for i in 0..package.len() {
        ...
        // Zip Slip 취약점 방어: 상대 경로 탈출(..) 및 절대 경로(/, \) 차단
        if name.contains("..") || name.starts_with('/') || name.starts_with('\\') {
            return err(format!("{label} contains illegal path traversal entry '{name}'"));
        }

        // 암호화 엔트리 차단: 비검사 악성 페이로드 은닉 방지
        if file.encrypted() {
            return err(format!("{label} contains encrypted entry '{name}'"));
        }

        // 단일 엔트리 크기 상한 검증 (Zip 폭탄 방어)
        let uncompressed = file.size();
        if uncompressed > HWPX_MAX_ENTRY_BYTES {
            return err(format!(
                "entry '{name}' in {label} ({} bytes) exceeds maximum entry limit",
...
mod tests {
    ...
    fn rejects_zip_slip_path_traversal() {
        ...
        assert!(check_package_size(&mut archive, "test").is_err());

        // Path traversal with leading "/"
        let mut buf2 = Vec::new();
        {
            let mut zw = ZipWriter::new(Cursor::new(&mut buf2));
            zw.start_file("/etc/passwd", SimpleFileOptions::default()).unwrap();
            zw.finish().unwrap();
        }
        let mut archive2 = ZipArchive::new(Cursor::new(buf2)).unwrap();
        assert!(check_package_size(&mut archive2, "test").is_err());
    }
}
